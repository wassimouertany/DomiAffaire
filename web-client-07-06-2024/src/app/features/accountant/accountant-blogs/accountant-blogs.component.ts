...
@Component({
  selector: 'app-accountant-blogs',
  templateUrl: './accountant-blogs.component.html',
  styleUrls: ['./accountant-blogs.component.css']
})
export class AccountantBlogsComponent  implements OnInit {
  Blogs: any;
  currentBlogID: any;
  ...
  getAllBlogs() {
    this.accountantService.getAccountantBlogs().subscribe({
      next: (data: any) => {
        console.log(data);
        // this.Blogs = data;
  ...
  removeBlogs() {
    this.accountantService.deleteBlog(this.currentBlogID).subscribe({
      next: (data: any) => {
